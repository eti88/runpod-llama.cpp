...
# Check if mmproj file needs to be downloaded
if [ ! -f "/app/models/${MMPROJ_FILENAME}" ] && [ -n "${MODEL_NAME:-}" ]; then
  echo "mmproj file not found at /app/models/${MMPROJ_FILENAME}; attempting to download..."
...
target_path = '/app/models/${MMPROJ_FILENAME}'

print(f'Downloading mmproj from HuggingFace: {repo_id}/{mmproj_filename}')

try:
    downloaded = hf_hub_download(repo_id=repo_id, filename=mmproj_filename)
    shutil.copy2(downloaded, target_path)
    print(f'mmproj saved to: {target_path}')
except Exception as e:
    print(f'mmproj download failed: {e}', file=sys.stderr)
    sys.exit(1)
"
  
  if [ $? -ne 0 ]; then
    echo "Failed to download mmproj file from HuggingFace" >&2
    exit 1
  fi
  
  echo "Successfully downloaded mmproj to /app/models/${MMPROJ_FILENAME}"
fi
...
exec /usr/local/bin/llama-server \
  -m "/app/models/${MODEL_FILENAME}" \
  --mmproj "/app/models/${MMPROJ_FILENAME}" \
  -ngl 999 \
  -fa on \
  --batch-size 2048 \
  --ubatch-size 2048 \
